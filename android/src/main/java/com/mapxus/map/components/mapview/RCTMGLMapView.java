...
import com.mapxus.map.components.annotation.RCTMGLMarkerView;
import com.mapxus.map.components.annotation.MarkerView;
import com.mapxus.map.components.annotation.MarkerViewManager;
import com.mapxus.map.components.camera.RCTMGLCamera;
import com.mapxus.map.components.images.RCTMGLImages;
import com.mapxus.map.components.location.LocationComponentManager;
import com.mapxus.map.components.location.RCTMGLNativeUserLocation;
import com.mapxus.map.components.mapview.helpers.CameraChangeTracker;
import com.mapxus.map.components.mapview.helpers.Insets;
import com.mapxus.map.components.styles.layers.RCTLayer;
import com.mapxus.map.components.styles.light.RCTMGLLight;
import com.mapxus.map.components.styles.sources.RCTMGLShapeSource;
...
import com.mapxus.map.events.MapChangeEvent;
import com.mapxus.map.events.MapClickEvent;
...
import com.mapxus.map.utils.GeoViewport;
...
public class RCTMGLMapView extends MapView implements OnMapReadyCallback, MapboxMap.OnMapClickListener,
        ...
        MapView.OnDidFinishLoadingStyleListener, MapView.OnStyleImageMissingListener, MapxusMapFeature {
    ...
    public void addFeature(View childView, int childPosition) {
        ...
        if (feature != null) {
            if (mQueuedFeatures == null || (childView instanceof RCTMGLPointAnnotation && mQueuedFeatures.isEmpty())) {
    ...
    @Override
    public void addToMap(RCTMapxusMap mapxusMap) {
        if (mapxusMap.getRctMapxusMapOptions() != null) {
            Double zoomLevel = mapxusMap.getRctMapxusMapOptions().getZoomLevel();
            if (zoomLevel != null) {
                mMap.moveCamera(CameraUpdateFactory.zoomTo(zoomLevel));
            } else if (mapxusMap.getRctMapxusMapOptions().getBuildingId() != null || mapxusMap.getRctMapxusMapOptions().getPoiId() != null) {
                mMap.moveCamera(CameraUpdateFactory.zoomTo(17.0));
            }
            Insets zoomInsets = mapxusMap.getRctMapxusMapOptions().getZoomInsets();
            if (zoomInsets != null) {
                mMap.moveCamera(CameraUpdateFactory.paddingTo(zoomInsets.getLeft(), zoomInsets.getTop(), zoomInsets.getRight(), zoomInsets.getBottom()));
            }
        }
    }

    @Override
    public void removeFromMap(RCTMapxusMap mapView) {

    }

    @Override
    public View getMapxusChildView() {
        return this;
    }
